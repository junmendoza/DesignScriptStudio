...
{
    class CodeBlockNode2 : VisualNode
    {
        ...
        internal static string GetInputSlotNameFromInputSlotIndex(List<Statement> statements, int slotIndex)
        {
            if (statements == null)
                throw new ArgumentNullException("statement");

            List<VariableSlotInfo> references = new List<VariableSlotInfo>();
            foreach (Statement statement in statements)
                references.AddRange(statement.References);
            return references.ElementAt(slotIndex).Variable;
        }

        internal static List<string> GetReferencedVariables(List<Statement> statements, bool includeTempVariables)
        {
            if (statements == null)
            ...
            foreach (Statement statement in statements)
                statement.GetReferenceVariables(referencedVariables, includeTempVariables);
        ...
        internal CodeBlockNode2(IGraphController graphController, string text)
        ...
        internal CodeBlockNode2(IGraphController graphController, SnapshotNode snapshotNode)
            : base(graphController, NodeType.CodeBlock)
        {
            this.Caption = string.Empty;
        ...
        internal CodeBlockNode2(IGraphController graphController) : base(graphController) { }
        ...
        internal override string GetSlotName(SlotType slotType, uint slotId)
        {
            ...
            if (slotType == SlotType.Input)
                return CodeBlockNode2.GetInputSlotNameFromInputSlotIndex(this.statements, slotIndex);
            else if (slotType == SlotType.Output)
                return CodeBlockNode2.GetOutputSlotNameFromOutputSlotIndex(this.outputLines, slotIndex);
            else
        ...
        internal override Point GetSlotPosition(Slot slot)
        {
            ...
            if (slot.SlotType == SlotType.Input)
                lineNumber = CodeBlockNode2.GetLineNumberFromInputSlotIndex(this.inputLines, slotIndex);
            else
            {
                slotPosition.X += this.Width;
                lineNumber = CodeBlockNode2.GetLineNumberFromOutputSlotIndex(this.outputLines, slotIndex);
            }
        ...
        internal override List<uint> GetSlotIdsByName(SlotType slotType, string variableName)
        {
            ...
            if (slotType == SlotType.Input)
            {
                List<int> inputSlotIndices = CodeBlockNode2.GetInputSlotIndicesByName(this.inputLines, variableName);
            ...
            {
                List<int> outputSlotIndices = CodeBlockNode2.GetOutputSlotIndicesByName(this.outputLines, variableName);
        ...
        {
            return CodeBlockNode2.GetDefinedVariables(this.outputLines, includeTemporaryVariables).Distinct().ToList<string>();
        }

        internal override List<string> GetReferencedVariables(bool includeTemporaryVariables)
        {
            return CodeBlockNode2.GetReferencedVariables(this.statements, includeTemporaryVariables).Distinct().ToList<string>();
        }
        ...
        private bool InputSlotIsVisible(int lineNumber, out int index)
        {
            ...
            index = CodeBlockNode2.GetInputSlotIndexFromLineNumber(this.inputLines, lineNumber);
            if (index != -1)
            {
        ...
        private bool OutputSlotIsVisible(int lineNumber, out int index)
        {
            ...
            index = CodeBlockNode2.GetOutputSlotIndexFromLineNumber(this.outputLines, lineNumber);
            if (index != -1)
            {
        ...
        private void SetInputSlotsAndLines(Dictionary<int, List<VariableLine>> newInputLines)
        {
            ...
            List<string> oldSetVariables = CodeBlockNode2.GetReferencedVariables(this.statements, true);
            List<string> newSetVariables = CodeBlockNode2.GetReferencedVariables(this.statements, true);
        ...
        private void SetOutputSlotsAndLines(List<VariableLine> newOutputLines)
        {
            ...
            //      newSlotIds   = { 302, ???, ??? }
            // 
            List<string> newVariables = CodeBlockNode2.GetDefinedVariables(newOutputLines, true);
