...
{
    internal class DSMeshData
    {
        Group currentGroup;
        public List<DSPoint> Vertices { get; private set; }
        public List<DSVector> Normals { get; private set; }
        ...
        public DSMeshData()
        {
            Vertices = new List<DSPoint>();
            Normals = new List<DSVector>();
        ...
        public bool AddVertex(String rawData)
        {
            ...
            if (!Double.TryParse(parts[0], out x) || !Double.TryParse(parts[1], out y) || !Double.TryParse(parts[2], out z))
                return false;
            Vertices.Add(new DSPoint(x, y, z,false));
            return true;
        }
        ...
        public DSSubDivisionMesh[] ConvertToSubDivisionMesh()
        {
            List<DSSubDivisionMesh> importedMeshes = new List<DSSubDivisionMesh>();
            foreach (var group in this.Groups)
            {
                List<int[]> faces = new List<int[]>();
                SortedList<int, DSPoint> sortedIndexPointList = new SortedList<int, DSPoint>();
                ...
                if (faces.Count == 0)
                    continue;
                DSPoint[] usedPointsList = new DSPoint[sortedIndexPointList.Count];
                sortedIndexPointList.Values.CopyTo(usedPointsList, 0);
                try
                {
                    importedMeshes.Add(new DSSubDivisionMesh(usedPointsList, faces.ToArray(), 0, true));
                }
