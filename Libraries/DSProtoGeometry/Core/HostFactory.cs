...
namespace Autodesk.DesignScript.Geometry
{
    class HostFactory
    {
        ...
        private HostFactory()
        {
            try
            {
                ...
                if(found)
                {
                    factoryFilePath = DSGeometryExtension.LocateFile(factoryFilePath);
                    persistentMgrFilePath = DSGeometryExtension.LocateFile(persistentMgrFilePath);
                }
                ...
                {
                    factoryFilePath = DSGeometryExtension.LocateFile("ProtoAcadGeometry.dll");
                    if (!File.Exists(factoryFilePath))
                    {
                        factoryFilePath = DSGeometryExtension.LocateFile("DSGeometry.dll"); //For nunit-testing
                        if (!File.Exists(factoryFilePath))
                ...
                if (!ExtensionApplicationStarted && mExtensionApplications.Count > 0)
                {
                    ...
                    {
                        app.OnBeginExecution(DSApplication.Instance.Session);
                    }
        ...
        {
            var session = DSApplication.Instance.Session;
            if (null == session)
                return false;
    ...
    public class DSGeometrySettings
    {
        private static IGeometrySettings mHostSettings = null;
        static DSGeometrySettings()
        {
            Reset();
        }

        private DSGeometrySettings() { }
        ...
        /// </summary>
        public static DSColor DefaultColor { get; set; }
