...
namespace Autodesk.DesignScript.Geometry
{
    class GeometryExpressionBuilder
    {
        ...
        public string WriteEntity(IPointEntity point, string paramName = null)
        {
            ...
            mExpression.AppendFormat("{0} = DSPoint.ByCoordinates({1}, {2}, {3});", paramName, point.X, point.Y, point.Z);
        ...
        public string WriteEntity(IVector vector, string paramName = null)
        {
            ...
            mExpression.AppendFormat("{0} = DSVector.ByCoordinates({1}, {2}, {3});", paramName, vector.X, vector.Y, vector.Z);
            mExpression.AppendLine();
            return paramName;
        }
        ...
        public string WriteEntity(ICoordinateSystemEntity cs, string paramName = null)
        {
            ...
            mExpression.AppendFormat("{0} = DSCoordinateSystem.ByOriginVectors({1}, {2}, {3}, {4});", paramName, origin, xaxis, yaxis, zaxis);
            mExpression.AppendLine();
            return paramName;
        }
        ...
        public string WriteEntity(ILineEntity line, string paramName = null)
        {
            ...
            mExpression.AppendFormat("{0} = DSLine.ByStartPointEndPoint({1}, {2});", paramName, start, end);
            mExpression.AppendLine();
            return paramName;
        }
        ...
        public string WriteEntity(ICircleEntity circle, string paramName = null)
        {
            ...
            mExpression.AppendFormat("{0} = DSCircle.ByCenterPointRadius({1}, {2}, {3});", paramName, center, circle.Radius, normal);
            mExpression.AppendLine();
            return paramName;
        }
        ...
        public string WriteEntity(IBSplineCurveEntity bspline, string paramName = null)
        {
            ...
            mExpression.AppendFormat("{0} = DSBSplineCurve.ByControlVertices({1}, {2}, {3});", paramName, cp, bspline.GetDegree(), bspline.GetIsPeriodic());
            mExpression.AppendLine();
            return paramName;
        }
        ...
        private string WriteSATImport(IGeometryEntity[] entities, string paramName)
        {
            ...
            mExpression.AppendFormat(@"{0} = DSGeometry.ImportFromSAT(""{1}"");", paramName, filePath);
            mExpression.AppendLine();
            return fileName;
        }
